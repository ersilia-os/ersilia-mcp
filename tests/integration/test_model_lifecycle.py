...
def test_model_complete_lifecycle(tmp_path):
    ...
    inspect_result = isaura_operations.inspect(model_id, ",".join(test_inputs))
    assert inspect_result["status"] == "ok", f"Inspect failed: {inspect_result}"
    assert inspect_result["num_cached"] == len(test_inputs), (
        f"Expected {len(test_inputs)} cached inputs, got {inspect_result}"
    )
