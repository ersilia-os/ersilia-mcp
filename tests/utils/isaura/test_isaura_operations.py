...
def test_inspect_reports_cached_and_missing(mock_cached):
    """Test inspect splits requested inputs into cached and missing."""
...
@patch(f"{_OPS}._inspect_cached")
def test_inspect_caps_reported_lists(mock_cached):
    """Test inspect caps the cached/missing lists at _MAX_REPORTED entries."""
    cached = [f"C{i}" for i in range(30)]
    mock_cached.return_value = cached
    result = inspect("eos3b5e", ",".join(cached))
    assert result["num_cached"] == 30
    assert len(result["cached"]) == isaura_operations._MAX_REPORTED
